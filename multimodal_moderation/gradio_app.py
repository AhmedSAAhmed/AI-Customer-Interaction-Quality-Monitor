...
class ChatSessionWithTracing:
    ...
    def _record_feedback(self, feedback: str, *, is_flagged: bool) -> None:
        with tracer.start_as_current_span("feedback") as span:
            span.set_attributes(
                {
                    "session.id": self.session_id,
                    "feedback.content": feedback,
                    "feedback.is_flagged": is_flagged,
                }
            )
    ...
    async def chat_with_gemini(self, message: dict, history: List, past_messages: List) -> Tuple[str, List, str]:
        ...
        with tracer.start_as_current_span("chat_turn", context=trace.set_span_in_context(self.conversation_span)) as span:
            ...
            span.set_attributes(
                {
                    "session.id": self.session_id,
                    "input.text.length": len(message.get("text", "")),
                    "input.files.count": len(message.get("files", [])),
                }
            )
            ...
            for key, value in message.items():
                ...
                if key == "text" and value:
                    ...
                    if not is_safe:
                        ...
                        self._record_feedback(feedback, is_flagged=True)
                ...
                # MODERATION STEP 2: Check media files
                    ...
                    for file_path in value:
                        ...
                        try:
                            ...
                            image_or_media_input = BinaryContent(
                                data=file_bytes,
                                media_type=mime_type,
                            )

                            prompt_parts.append(image_or_media_input)
            ...
            try:
                if MOCK_AI:
                    response = (
                        "Mock customer: I'm frustrated that my ACME Power Widget Pro shut down, "
                        "but I appreciate your help. What can you do for me?"
                    )
                    span.set_attribute("mock_ai", True)
                    logger.info("Mock response generated")
                    self._record_feedback(safety_message, is_flagged=False)
                    return response, past_messages, safety_message
